...
function Orphans() {
  ...
  const handleReportUpload = async () => {
    if (!selectedOrphanId || !reportFile || !supportMessages[selectedOrphanId]) {
      alert("Please select an orphan, enter a message, and attach a report file");
      return;
    }

    const form = new FormData();
    form.append("orphan_id", selectedOrphanId);
    form.append("report_text", supportMessages[selectedOrphanId]);
    ...
    try {
      ...
      setReportFile(null);
      setSupportMessages((prev) => ({ ...prev, [selectedOrphanId]: "" }));
                  ...
                  <td className="border px-4 py-2">
                    <div className="space-y-1">
                      ...
                      <textarea
                        ...
                        value={supportMessages[orphan.id] || ""}
                        onChange={(e) =>
                          setSupportMessages({ ...supportMessages, [orphan.id]: e.target.value })
                        }
                      />
